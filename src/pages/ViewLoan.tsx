...
const ViewLoan = () => {
  ...
  useEffect(() => {
    const cli: any = localStorage.getItem("product");
    setProduct(cli);
    axios
      .get(baseURL, {
        headers: {
    ...
    <>
      <Container className="bg-yellow-100 mt-10 mb-20 pb-20 rounded-lg drop-shadow-lg">
        <Breadcrumbs className="p-5" aria-label="breadcrumb">
          ...
          <Typography className="text-black">Loan Products List</Typography>
        </Breadcrumbs>
        ...
        <div className="flex flex-row ">
          ...
          <div className="float-right">
            <Link to="/Products/LoanProduct/Create">
              ...
              <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {rows.map((loan: any) => (
                      <tr key={loan.id}>
                        {loan.productType.toLowerCase() === "loan" ? (
                          <Loanlist loan={loan} />
                        ) : (
                          <></>
                        )}
